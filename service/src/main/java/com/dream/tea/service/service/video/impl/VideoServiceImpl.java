...
public class VideoServiceImpl implements VideoService {
    ...
    @Override
    public VideoList getById(Long id) {
        return videoListMapper.getById(id);
    }

    @Override
    public List<VideoList> getByVideoListIdList(List<Long> videoListIdList) {
        if (CollUtil.isEmpty(videoListIdList)) {
            return Collections.emptyList();
        }
        return videoListMapper.getByVideoListIdList(videoListIdList);
    }
}
